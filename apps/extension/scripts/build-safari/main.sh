...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            info "Usage: $0 [--dmg|--rebuild-dmg] [--signed] | [--appstore]"
            exit 1
            ;;
    esac
done
...
# - Unsigned rebuild-dmg: create DMG = 1
# - Signed rebuild-dmg: create DMG, notarize, staple = 3
...
if [ "$BUILD_TYPE" = "dmg" ] || [ "$BUILD_TYPE" = "rebuild-dmg" ]; then
    ...
    if [ "$SIGNED" = true ]; then
        ...
        if [ "$BUILD_TYPE" = "rebuild-dmg" ]; then
            TOTAL_STEPS=3
            BUILD_DESCRIPTION="Rebuild DMG (signed)"
        else
            TOTAL_STEPS=7
        ...
        if [ "$BUILD_TYPE" = "rebuild-dmg" ]; then
            TOTAL_STEPS=1
            BUILD_DESCRIPTION="Rebuild DMG (unsigned)"
        else
            TOTAL_STEPS=4
...
if [ "$BUILD_TYPE" = "rebuild-dmg" ]; then
    ...
    if [ ! -d "$APP_PATH" ]; then
        ...
        info "The --rebuild-dmg option requires an existing production build."
        info "Run with --dmg instead to perform a full build."
        exit 1
    fi
    
    # If signed, verify the code signature
    if [ "$SIGNED" = true ]; then
        info "Verifying existing code signature..."
        if ! codesign --verify --deep --strict "$APP_PATH" 2>/dev/null; then
            echo -e "${RED}Error: Code signature verification failed for $APP_PATH${NC}"
            info "The existing build may be unsigned or have an invalid signature."
            info "Run with --dmg --signed instead to perform a full signed build."
            exit 1
        fi
        
        # Check if it was signed with the correct identity
        EXISTING_CODESIGN=$(codesign -dv --verbose=4 "$APP_PATH" 2>&1)
        EXISTING_AUTHORITY=$(echo "$EXISTING_CODESIGN" | grep "Authority=" | head -1 | sed 's/Authority=//')
        if [[ "$EXISTING_AUTHORITY" != *"$DEVELOPER_NAME"* ]]; then
            echo -e "${RED}Error: Existing build was signed with wrong identity${NC}"
            info "Found: $EXISTING_AUTHORITY"
            info "Expected: $DEVELOPER_ID_APP"
            info "Run with --dmg --signed instead to perform a full signed build."
            exit 1
        fi
        
        # Check for hardened runtime (required for notarization)
        if ! echo "$EXISTING_CODESIGN" | grep -q "flags=.*runtime"; then
            echo -e "${RED}Error: Existing build does not have hardened runtime enabled${NC}"
            info "This is required for notarization."
            info "Run with --dmg --signed instead to perform a full signed build."
            exit 1
        fi
        
        success "Existing build signature verified: $EXISTING_AUTHORITY"
    fi
fi
...
# =============================================================================
# Full build steps (skipped for rebuild-dmg)
# =============================================================================
